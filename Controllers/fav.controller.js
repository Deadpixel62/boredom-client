...
favController.findByType = (req, res) => {
    favList.find({ type: req.params.type })
    .exec(function (err, product) {
        if (err){
            ...
            return res.status(500).send({
                message: "Error retrieving Products with given favorite type: " + req.params.type
            });
        }          
        res.send(product);
    }); 
...
favController.findByNom = (req, res) => {
    favList.find({activity : { $regex : req.params.activity }})
    .exec(function (err, product) {
        if (err){
            if(err.kind === 'String') {
                return res.status(404).send({
                    message: "Products not found with given name " + req.params.activity
                });                
            }
            return res.status(500).send({
                message: "Error retrieving Products with given favorite activity : " + req.params.activity
            });
        }
...
favController.findByName = async function (req,res) {
    let activity;
    try {
        activity = await favList.find({ activity : { "$regex": req.params.activity } });
        res.send(activity);
    } catch (error) {
        res.status(500).send(error)
    }
